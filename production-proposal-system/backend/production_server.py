...
class SystemConfig:
    ...
    LITEFS_MOUNT_POINT = "/mnt/litefs"
    LITEFS_REPLICAS = ["replica1", "replica2", "replica3"]

    # Litestream Configuration
    LITESTREAM_BACKUP_DIR = "backup"
    LITESTREAM_RETENTION_DAYS = 30
    LITESTREAM_BATCH_SIZE = 1024 * 1024
...
class DatabaseManager:
    ...
    def initialize_database(self):
        ...
        self.primary_conn = sqlite3.connect(SystemConfig.DATABASE_URL, check_same_thread=False)
    ...
    def setup_replication(self):
        ...
        for i, replica_id in enumerate(["local_replica1", "local_replica2", "local_replica3"]):
            ...
            replica_db = sqlite3.connect(f"{replica_db_path}/proposal_system.db", check_same_thread=False)
    ...
    def copy_schema_to_replica(self, replica_db):
        """Copy schema to replica"""
        schema_sql = "SELECT sql FROM sqlite_master WHERE type='table' AND sql NOT LIKE 'sqlite_%'"
        ...
        # Copy indexes
        index_sql = "SELECT sql FROM sqlite_master WHERE type='index' AND sql NOT LIKE 'sqlite_%'"
            ...
            with replica_db:
                for proposal in changes['proposals']:
                    ...
                    """, (
                        ...
                        proposal['created_by'], proposal['created_at'], proposal['updated_at'],
                        proposal.get('ai_generated', 0), proposal.get('ai_score'),
                        proposal.get('compliance_score'), datetime.utcnow().isoformat()
                    ))
        except Exception as e:
    ...
    def cleanup_old_backups(self):
        ...
        try:
            backup_dir = SystemConfig.LITESTREAM_BACKUP_DIR
            cutoff_date = datetime.now() - timedelta(days=SystemConfig.LITESTREAM_RETENTION_DAYS * 2)
    ...
    try:
        ...
        if not admin_user:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (str(uuid.uuid4()), 'admin', 'admin@proposal-system.com',
                  'System Administrator', password_hash, 'admin', now, now))
...
if __name__ == '__main__':
    ...
    print(f"🔄 Replication: {len(SystemConfig.LITEFS_REPLICAS)} replicas")
    ...
    print(f"   Backups: {len([f for f in os.listdir(SystemConfig.LITESTREAM_BACKUP_DIR) if f.endswith('.db')])}")
...
## 3. Frontend dengan Read Replica Routing

"""
Production Proposal System Backend v2
Focus on: WAL Mode, Replication, Disaster Recovery, Performance Monitoring
SQLite3 + Flask = Production Ready
"""
...
# Database Manager with Production Features
class ProductionDatabase:
    ...
    def initialize_database(self):
        ...
        try:
            self.primary_conn = sqlite3.connect(
                ProductionConfig.DATABASE_URL, check_same_thread=False
            )
            ...
            self.primary_conn.execute("PRAGMA cache_size=" + str(ProductionConfig.CACHE_SIZE))
            self.primary_conn.execute("PRAGMA temp_store=" + ProductionConfig.TEMP_STORE)
            self.primary_conn.execute("PRAGMA mmap_size=268435456")  # 256MB memory-mapped I/O
    ...
    def setup_replication(self):
        ...
        logger.info(f"Setting up {ProductionConfig.READ_REPLICA_COUNT} read replicas...")

        for i in range(ProductionConfig.READ_REPLICA_COUNT):
            ...
            try:
                ...
                replica_db.execute("PRAGMA cache_size=15000")  # Larger cache for reads
    ...
    def setup_backup(self):
        ...
        os.makedirs(ProductionConfig.BACKUP_DIR, exist_ok=True)
    ...
    def cleanup_old_backups(self):
        ...
        try:
            backup_dir = ProductionConfig.BACKUP_DIR
            cutoff_date = datetime.now() - timedelta(days=ProductionConfig.BACKUP_RETENTION_DAYS * 2)
    ...
    def get_connection(self, read_only=False):
        ...
        if read_only and self.read_replicas:
            # Route reads to least recently synced replica
    ...
    def increment_write_wps(self):
        ...
        # Check limit
        if self.write_wps_counter > ProductionConfig.MAX_WRITE_WPS:
            logger.warning(f"Write WPS exceeded: {self.write_wps_counter}/{ProductionConfig.MAX_WRITE_WPS}")
    ...
    try:
        ...
        return jsonify({
            ...
            'backup': {
                'enabled': True,
                'backup_dir': ProductionConfig.BACKUP_DIR,
                'backup_count': len([f for f in os.listdir(ProductionConfig.BACKUP_DIR) if f.endswith('.db')]) if os.path.exists(ProductionConfig.BACKUP_DIR) else 0
            },
            'performance': {
                'wps_current': db.get_current_wps(),
                'wps_limit': ProductionConfig.MAX_WRITE_WPS,
                'uptime': perf_monitor.get_uptime(),
                'metrics': perf_monitor.metrics
            },
            'features': {
                'wal_mode': ProductionConfig.WAL_MODE,
...
def get_metrics():
    ...
    return jsonify({
        ...
        'system': {
            ...
            'backup_count': len([f for f in os.listdir(ProductionConfig.BACKUP_DIR) if f.endswith('.db')]) if os.path.exists(ProductionConfig.BACKUP_DIR) else 0,
            'config': {
                'max_write_wps': ProductionConfig.MAX_WRITE_WPS,
                'backup_retention_days': ProductionConfig.BACKUP_RETENTION_DAYS,
                'read_replica_count': ProductionConfig.READ_REPLICA_COUNT
            }
        }
    })
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"   Backups: {len([f for f in os.listdir(ProductionConfig.BACKUP_DIR) if f.endswith('.db')]) if os.path.exists(ProductionConfig.BACKUP_DIR) else 0}")
    except Exception as e:
...
- **Target Latency**: ~200ms untuk optimal performance

### ✅ **3. Disaster Recovery (Litestream-style)**
- **Automatic Backup** setiap menit
- **30 Day Retention** dengan archive
- **WAL Backup** untuk crash recovery
- **Cleanup** otomatis untuk old backups
...
- **Backup Status**: File size, retention status
